        ...
        <div className="result-page">
            ...
            <main className="results-container">
                ...
                {votes.map((vote, index) => (
                    <div 
                        key={vote.cId} 
                        className={`result-card ${timeElapsed >= 50 ? highlightClass(index) : ''}`}
                    >
                        <h2>{vote.cName}</h2>
                        <p className="votes">{vote.total_votes} Votes</p>
                        {timeElapsed >= 50 && index === 0 && <span className="badge gold">🥇 1st</span>}
                        {timeElapsed >= 50 && index === 1 && <span className="badge silver">🥈 2nd</span>}
                        {timeElapsed >= 50 && index === 2 && <span className="badge bronze">🥉 3rd</span>}
                    </div>
                ))}
            </main>
        </div>
