...
const Result = () => {
    ...
    useEffect(() => {
        ...
        const voteInterval = setInterval(fetchVotes, 20);
        ...
        <div className="result-page">
            ...
            <main className="results-container">
                ...
                {votes.map((vote, index) => (
                    <div 
                        key={vote.cId} 
                        className={`result-card ${timeElapsed >= 20 ? highlightClass(index) : ''}`}
                    >
                        <h2>{vote.cName}</h2>
                        <p className="votes">{vote.total_votes} Votes</p>
                        {timeElapsed >= 20 && index === 0 && <span className="badge gold">🥇 1st</span>}
                        {timeElapsed >= 20 && index === 1 && <span className="badge silver">🥈 2nd</span>}
                        {timeElapsed >= 20 && index === 2 && <span className="badge bronze">🥉 3rd</span>}
                    </div>
                ))}
            </main>
        </div>
